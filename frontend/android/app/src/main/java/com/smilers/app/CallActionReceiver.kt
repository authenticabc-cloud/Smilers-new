...
class CallActionReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        when (intent.action) {
            "com.smilers.app.ACTION_ANSWER_CALL" -> handleAnswer(context, intent)
            "com.smilers.app.ACTION_DECLINE_CALL" -> handleDecline(context, intent)
            else -> {
    ...
    companion object {
        // sml-010: how long the in-app silent-decline screen gets to confirm
        // before the old native-only path runs as a backstop. Matches the
        // ~3-4s window agreed for the fallback.
        private const val DECLINE_FALLBACK_DELAY_MS = 3_500L

        /**
         * sml-011: shared by the BroadcastReceiver (onReceive above, still wired for
         * the notification body's own tap handling / legacy compatibility) AND
         * CallActionTrampolineActivity — the invisible activity that the Answer
         * notification button now launches directly via PendingIntent.getActivity()
         * so the app reliably opens even from a fully killed process on OEMs
         * (Samsung/Motorola) whose background-activity-launch restrictions blocked
         * a plain BroadcastReceiver's own startActivity() call.
         */
        fun handleAnswer(context: Context, intent: Intent) {
            ...
            if (targetUrl.isNotEmpty()) {
                try {
                    ...
                    Log.d("SmilersCall", "CallActionReceiver: launched answer for callId=$callId")
                } catch (e: Exception) {
                    Log.w("SmilersCall", "CallActionReceiver: answer launch failed: ${e.message}")
                }
            }
        }

        /** sml-011: see handleAnswer's doc — same reasoning, now used by the Decline button. */
        fun handleDecline(context: Context, intent: Intent) {
            val callId = intent.getStringExtra("callId") ?: ""
            ...
            // sml-010: PRIMARY path is now an in-app, fully-authenticated decline.
            // Launch the app straight to a headless screen (no call UI, no ringtone —
            // see app/silent-decline.tsx) that declines over the live Convex session
            // every other in-app action already trusts, instead of a stored token or
            // a push relay back to the caller. This sidesteps both failure classes
            // entirely for the common case where the app can reach the foreground.
            try {
                context.startActivity(Intent(context, MainActivity::class.java).apply {
                    action = Intent.ACTION_VIEW
                    data = Uri.parse(
                        "smilers://silent-decline?callId=${Uri.encode(resolvedCallId)}" +
                            "&conversationId=${Uri.encode(convId)}"
                    )
                    addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_CLEAR_TOP or Intent.FLAG_ACTIVITY_SINGLE_TOP)
                })
                Log.d("SmilersCall", "CallActionReceiver: launched silent-decline for callId=$resolvedCallId")
            } catch (e: Exception) {
                Log.w("SmilersCall", "CallActionReceiver: silent-decline launch failed: ${e.message}")
            }

            // Bounded backstop: the OLD native-only decline path (unchanged logic below),
            // now scheduled rather than fired immediately. Cancelled by
            // SmilersCallModule.cancelDeclineFallback() the moment the in-app decline
            // confirms; only actually runs if that doesn't happen in time (auth not
            // ready, network blip, OS didn't grant foreground time, etc.).
            val fallbackKey = resolvedCallId.ifEmpty { convId }
            val fallbackRunnable = Runnable {
                runDeclineFallback(context, callId, convId, resolvedCallId, callerUserId, backendUrl)
            }
            SmilersCallNotificationService.scheduleDeclineFallback(
                fallbackKey, fallbackRunnable, DECLINE_FALLBACK_DELAY_MS,
            )
        }

        /**
         * sml-010: the OLD native-only decline path — unchanged logic, extracted verbatim
         * from what used to run immediately on every Decline tap. Now only runs as a
         * bounded backstop (see DECLINE_FALLBACK_DELAY_MS) if the in-app silent-decline
         * screen hasn't confirmed by then. Runs on its own background thread since the
         * caller (a Handler callback) fires on the main thread.
         */
        private fun runDeclineFallback(
        context: Context,
        callId: String,
        convId: String,
        resolvedCallIdIn: String,
        callerUserIdIn: String,
        backendUrlIn: String,
    ) {
        Thread {
            var callerUserId = callerUserIdIn
            var backendUrl = backendUrlIn
            val resolvedCallId = resolvedCallIdIn
            try {
                Log.d("SmilersCall", "CallActionReceiver: decline fallback firing (in-app decline did not confirm in time) callId=$callId convId=$convId")
                ...
                Log.w("SmilersCall", "CallActionReceiver: decline fallback thread unexpected error: ${e.message}")
            }
        }.start()
    }
