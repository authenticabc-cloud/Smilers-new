...
- **App Lock removed:** `src/components/AppLockGate.tsx` `loadAppLockState()` now always returns `enabled:false/hasPin:false` → the lock never engages app-wide (settings toggle is inert; acceptable).
