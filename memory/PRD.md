...
- ✅ Deployment hardening for Emergent/EAS Android builds: moved `typescript` and `@babel/core` into production dependencies so Expo CLI checks still work when the builder omits devDependencies, switched the committed `packageManager` to `npm@10.8.2` to match the deployment runtime, and added `frontend/eas.json` with `cli.appVersionSource` + `app-bundle` profile so the pipeline stops generating that config dynamically
