...
- FIX: new `scripts/patch-expo-modules-race.js` (wired into package.json postinstall+prepare, idempotent) patches `node_modules/expo-modules-core/ios/Core/ExpoBridgeModule.mm` to call `[_appContext useModulesProvider:@"ExpoModulesProvider"]` EARLY — right before `_appContext._runtime = [EXJavaScriptRuntimeManager runtimeFromBridge:...]` in setRuntimeExecutor (RN≥0.74/bridgeless), setBridge (<0.74), and the installModules fallback. `ModuleRegistry.register(holder:)` is `registry[name]=holder` (idempotent overwrite), and the existing legacyProxyDidSetBridge call still runs so legacy modules keep their legacy-registry linkage. Result: registry fully populated (51) before JS runs → ExpoLinking always resolvable → no race.
- Kept iter-456 Podfile (static_library pre_install + DCS=NO) AS-IS because that exact config is KNOWN to BUILD successfully (user's last build compiled); the race patch is the actual behavioral fix. Kept iter-456 skia xcrun patch + probes.
- Patch applied+verified locally (3 insert sites, idempotent re-run confirmed, .mm braces balanced). ⚠️ NATIVE — one EAS iOS build. SUCCESS = boots past splash to Sign In; confirm via device beacons (`app-migration-75.emergent.host/api/diagnostic-logs/recent`): TM-PROBE expoBefore≈50, MODULE-INVENTORY present=20/20, no ExpoLinking GLOBAL-ERROR.

## iter-456 (Jun 2026): iOS ROOT CAUSE (mis)attributed to DCS + xcrun skia patch
- CLEAN device data at last (earlier reads were POLLUTED by web-preview beacons; os=web/hermes=false = the browser preview, NOT the device). Real device (posts to `app-migration-75.emergent.host`): native `modulesProvider: count=51 providerClass=ExpoModulesProvider cfBundleName=Smilers` (provider RESOLVES fine, my iter-455 "empty provider" theory was WRONG), but JS `TM-PROBE os=ios hermes=true bridgeless=y expoBefore=6 emcResolvable=true` → only ~6 of 51 modules reach `globalThis.expo.modules`, `present=2/20`, and `GLOBAL-ERROR: Cannot find native module 'ExpoLinking'` → fatal at expo-router init → splash hang.
- ROOT CAUSE (definitive): under `ios.useFrameworks=static` + New Arch, the Swift module-class IMPLEMENTATIONS are DEAD-CODE-STRIPPED from the app binary even though getModuleClasses() lists all 51 metatypes — so most modules fail to register into the JS runtime. Classic static-frameworks stripping. (static_library conversion from iter-453 only rescued ~6.)
- FIX (3 parts):
  1. `ios/Podfile` post_install: re-added `DEAD_CODE_STRIPPING=NO` on the app target Release (proven in iter-451 to register modules). NO `-all_load` (that clashes with skia's 9 shared static libs). Kept the static_library pre_install too.
  2. `scripts/patch-rn-skia-webp.js`: DCS=NO's only known side effect is duplicate libwebp (skia bundled vs SDWebImage). The patch strips skia's bundled libwebp but was FAILING on EAS (`ar: file format not recognized` — libskia.a is a 0xCAFEBABE FAT archive). Fixed to call all tools via `xcrun` (llvm-ar/lipo, correct Apple toolchain + PATH); skips gracefully on the Linux sandbox (xcrun ENOENT, exit 0).
  3. `index.js` TM-PROBE: now also logs `expoMods=[...]` (the names of the modules that DID register) to confirm the fix (expect ~20+ incl. ExpoLinking).
- Podfile `ruby -c` OK; skia patch `node -c` OK + skips clean on Linux; AppDelegate braces balanced.
- ⚠️ NATIVE — one EAS iOS build. Expected: app boots past splash to Sign In. If the archive fails at Ld with libwebp dupes, the xcrun skia patch still isn't running on EAS → need the exact patch error next. Read device beacons at `app-migration-75.emergent.host/api/diagnostic-logs/recent` (TM-PROBE expoMods + present count).

## iter-455 (Jun 2026): iOS 0/20 ROOT CAUSE isolated to `AppContext.modulesProvider()` empty-fallback — decisive native probe added
- Ruled OUT this round (hard evidence): (1) NOT a linker/dead-strip issue — iter-453 build LINKED clean; (2) NOT expo-dev-client — expo-dev-launcher/expo-dev-menu are `debugOnly:true` (auto-excluded from Release); native beacons (readable at `app-migration-75.emergent.host`) show a CLEAN launch: didFinishLaunching→embeddedBundle found→startReactNative before/after→bundleURL=embedded main.jsbundle; (3) NOT a web/wrong-platform bundle — built the iOS JS bundle locally (`expo export:embed`, 4199 modules) and confirmed it contains the NATIVE expo-modules-core (`"Unable to install Expo modules"`, `installModules`), not the web polyfill.
- TM-PROBE from device: `expoBefore=0 tmProxy=absent emcResolvable=false ...` → `globalThis.expo` EXISTS but `.modules` is EMPTY, and `TurboModuleRegistry.get('ExpoModulesCore')` is null. Traced native resolution: `AppContext.modulesProvider(withName:)` (expo-modules-core `AppContext.swift`) does `NSClassFromString("\(CFBundleName).ExpoModulesProvider")` → else `NSClassFromString("ExpoModulesProvider")` → else **empty `ModulesProvider()` (0 classes)**. Empty `globalThis.expo.modules` == it hit the empty fallback. Generated provider locally: it correctly lists all 50 `*.self` classes, so generation is fine — the class just isn't being FOUND under the app's Swift module at runtime (a known `use_frameworks static` provider-resolution failure). CFBundleName=`$(PRODUCT_NAME)`=Smilers, app module=Smilers (should match), so the provider likely isn't landing in the "Smilers" module during pod-install `project_integrator`.
- ACTION (`ios/Smilers/AppDelegate.swift`): added `import ExpoModulesCore` + a decisive native beacon after startReactNative printing `AppContext.modulesProvider().getModuleClasses().count`, the resolved provider class name, and CFBundleName. count=0 ⇒ provider not resolvable in app module (fix: force ExpoModulesProvider.swift into the Smilers target); count=50 ⇒ resolves, empty registry is a later registration/JS-exposure bug. Native beacon is READABLE by main agent at the old host. Low-risk read-only probe.
- ⚠️ NATIVE — one more EAS iOS build; then read BOTH `app-migration-75.emergent.host/api/diagnostic-logs/recent` (modulesProvider count) and this pod's `/api/diagnostic-logs/recent` (TM-PROBE).

## iter-454 (Jun 2026): iOS build now LINKS ✅ but still 0/20 modules → TRUE root cause = ExpoModulesCore JSI host never installed (+ probe/auto-fix)
- iter-453 WORKED for the LINKER: user's EAS iOS build succeeded (no 78 dupes) and pushed to TestFlight. BUT app STILL hangs on splash. Pulled the device's JS boot beacons from THIS pod's backend (`GET /api/diagnostic-logs/recent`, EXPO_PUBLIC_BACKEND_URL is baked into the IPA): `MODULE-INVENTORY present=0/20` — TOTAL, all 20 incl. core ExpoConstants/ExpoCrypto missing, `NativeModule=function` (expo-modules-core JS loaded). Boot now reaches `after-router-entry` (null-safe shim prevents the hard crash) but nothing works with 0 modules.
- KEY REALIZATION: prior `-all_load` builds NEVER actually linked (78 dupes), so "-all_load → present=20/20" was NEVER verified — it was always a guess. And 0/20 TOTAL (not partial) rules OUT Swift-class dead-stripping. Traced the JS path: `requireOptionalNativeModule` → `globalThis.expo?.modules?.[name]`; `globalThis.expo` is installed by `ensureNativeModulesAreInstalled()` → `TurboModuleRegistry.get('ExpoModulesCore').installModules()`. 0/20 total ⇒ that install never happened (either the ExpoModulesCore TurboModule isn't resolvable in the New-Arch registry, or installModules is never triggered).
- ACTION (`index.js`, JS-only): added a `TM-PROBE` beacon that reports `expoBefore / tmProxy / emcResolvable / installFn / getErr / installTried / installErr / expoAfter`. It DISAMBIGUATES: (a) emcResolvable=false ⇒ TurboModule not registered/linked (native fix needed); (b) resolvable + manual installModules() populates expoAfter=20 ⇒ it was an ORDERING problem and the probe ALSO FIXES it (it calls installModules() at index.js top-level, before expo-router imports). So this next build either boots (ordering) or tells us precisely why (linking).
- Kept iter-453's static-library Podfile change (harmless, keeps the build linking). ⚠️ NATIVE — needs ONE more EAS iOS build. After it, read `/api/diagnostic-logs/recent` for the `TM-PROBE` line.

## iter-453 (Jun 2026): iOS splash-hang / 78-duplicate-symbols DEADLOCK broken — Expo modules → static LIBRARIES (drop `-all_load`)
- ROOT CAUSE of the deadlock nailed: `-all_load` (the only thing that made Expo modules register / stopped the present=0/20 splash hang) is FUNDAMENTALLY INCOMPATIBLE with `@shopify/react-native-skia`. Skia ships **9 separate static libs** (`libskia/libsvg/libskottie/libsksg/libskparagraph/libskshaper/libskunicode_*/libpathops`.a) that SHARE objects; `-all_load` force-loads every member of all of them → guaranteed duplicate symbols (that's the skia half of the 78; the other half is RN's `React_Fabric` vs `React_FabricComponents` codegen overlap, also only surfaced by `-all_load`). So NO amount of surgical stripping saves `-all_load` — dropping it is the only correct move.
- FIX (`ios/Podfile`): added a `pre_install` hook that overrides `build_type => Pod::BuildType.static_library` for every Expo module pod (name `^Expo`, `^EX[A-Z]`, `expo-dev-menu`, `ReactAppDependencyProvider`). Static-LIBRARY linkage merges the module objects into the app binary so `ExpoModulesProvider.getModuleClasses()` force-loads the Swift classes normally (present=20/20) WITHOUT `-all_load`. This is Expo's OWN technique (`expo-modules-autolinking` `framework_modules_to_patch` does exactly this for core pods, but only when `RCT_USE_PREBUILT_RNCORE=1` — we build RN from source so that gate is off, hence the manual hook). REMOVED the old `withExpoModulesForceLoad` post_install block entirely (no more `-all_load`, no `DEAD_CODE_STRIPPING=NO`). All non-Expo pods (skia/Stream/WebRTC) stay static frameworks — unchanged.
- Note vs the ask_human plan: the `EXPO_USE_PRECOMPILED_MODULES=0` flag does NOT exist for iOS Expo modules in autolinking 3.0.26 (only Android has precompiled artifacts; iOS Expo modules already build from source). The static-library conversion is the correct realization of the same goal ("modules link/register without `-all_load`").
- The `patch-rn-skia-webp.js` still logs `ar: file format not recognized` on EAS (libskia.a is a FAT/0xCAFEBABE archive that BSD `ar` rejects) but it's now IRRELEVANT: without `-all_load` + with default dead-code-stripping, the linker dedups the incidental webp copy automatically (as in every pre-iter-451 build). Left the script untouched (never throws, exits 0).
- Podfile `ruby -c` = Syntax OK. ⚠️ NATIVE-ONLY — validate with ONE EAS iOS build. Success = pod install passes + archive links (no 78 dupes) + app boots past splash to Sign In. Risk: a specific Expo module misbehaving as a static lib under use_frameworks would fail at `pod install` (fast failure → next iteration).

## iter-452 (Jun 2026): iOS splash hang ROOT-CAUSED from device boot logs → `-all_load` fix for present=0/20
- BREAKTHROUGH: pulled live boot logs from the TestFlight device via `GET https://app-migration-75.emergent.host/api/diagnostic-logs/recent`. Sequence: AppDelegate `didFinishLaunching`→`startReactNative after`→ index.js `entry-start`→`after-messageReplyTask`→`after-streamIosPush`→ then FATAL `JSBOOT GLOBAL-ERROR: Cannot find native module 'ExpoLinking'`. So build+launch are fine; the hang is expo-modules-core registering 0 native modules (present=0/20) on the static-framework Release build.
- AppDelegate.swift is CORRECT (stock SDK54 ExpoAppDelegate + ExpoReactNativeFactory + ExpoReactNativeFactoryDelegate). `ios.buildReactNativeFromSource:"true"` already set. Root cause is `ios.useFrameworks=static`: each Expo module is its own static framework and the linker never pulls its Swift classes (archive-member selection), so ExpoModulesProvider finds none.
- Why prior fixes failed: `DEAD_CODE_STRIPPING=NO` was on the APP target but module classes live in SEPARATE framework binaries (can't be reached). Path-based `-force_load`/`-Wl,-force_load,<path>` fail EAS "Build input files cannot be found" (Xcode new-build-system input validation).
- FIX (Podfile post_install, app target Release): append `-all_load` to `OTHER_LDFLAGS` (keep `$(inherited)`), keep `DEAD_CODE_STRIPPING=NO`. `-all_load` force-loads ALL static-framework archive members via a single flag with NO paths → no input validation → all Expo module Swift classes present → present=20/20 → boot reaches Sign In. Known skia/libwebp duplicate already removed by `scripts/patch-rn-skia-webp.js` (iter-451).
- ⚠️ NATIVE-ONLY. Validate with ONE EAS iOS build. Risk: `-all_load` could surface OTHER duplicate symbols (would fail at Ld with a clear list → strip those next). Diagnostic beacons can re-read boot state after install.
...
## iter-444 (Jun 2026): Call tones still silent after rebuild → re-enabled on-device Diagnostic Logs to triage
- User rebuilt + tested iter-442 (SIGNALLING usage + full volume) → Connected/Ciaooo STILL silent. KEY INSIGHT: every recent user-confirmed fix (self-view iter-440, scroll iter-441, phone-verify iter-443) is PURE JS — none prove `scripts/patch-incallmanager.js` (which adds the native `playInCallSound`) actually ran in the Emergent build. So the tones could be failing because (a) the native patch wasn't applied → `playInCallSound` missing → JS no-ops, (b) the connect/hangup tone effects never fire, or (c) true audibility during the WebRTC session.
- The iter-442 `recordDiagnostic({tag:'TONE', … nativeMethod=<bool>})` already writes to the on-device Diagnostic Logs (persisted in AsyncStorage) AND POSTs to the user's PRODUCTION backend — which this FORK pod can't read (different backend URL). So re-enabled the hidden **Diagnostic Logs** row in `app/settings.tsx` (Settings → Diagnostic Logs) so the user can view/share the TONE entries after a test call.
- Decisive triage from the next build: `playCall…Tone called · nativeMethod=true` + silent → audibility (fix = app-owned native player / different stream). `nativeMethod=false` → patch not applied (fix = move the tone into the committed ios/android native code instead of patching node_modules). No `TONE` entry → the connect/hangup effect isn't firing.
- No blind native change this round (avoids another wasted build-guess loop). Lint clean; app boots to Sign In.


## iter-443 (Jun 2026): Phone verification no longer forced on in-place UPDATES (only true reinstall)
- User did an IN-PLACE APK update (no uninstall) yet was re-verified — this was the iter-439 one-time SecureStore→AsyncStorage marker migration (the AsyncStorage marker didn't exist yet, so verification fired once). User asked: verify on true reinstall, NEVER on update, and approved using the server `phoneVerified` flag as a fallback.
- **Fix (`settingsStorage.ts`):** new `DEVICE_PROVISIONED_KEY` (AsyncStorage — wiped on uninstall on BOTH platforms) + `markDeviceProvisioned()` + `resolveInstallVerified(serverPhoneVerified)`. Logic: (1) install marker `=== 'true'` → verified; (2) marker missing BUT server `phoneVerified === true` AND the device was previously provisioned (proof local data survived = UPDATE, since a reinstall wipes AsyncStorage) → treat verified + re-persist the marker. A genuine reinstall wipes everything → fallback can't fire → verification still required (iter-439 preserved).
- **Wiring:** `phone-verify.tsx` resolves via `resolveInstallVerified(me?.phoneVerified)` (re-runs when `me` loads) and calls `markDeviceProvisioned()` after a successful OTP verify. `(tabs)/_layout.tsx` uses the same resolver, marks the device provisioned once verified (so ALL existing verified users get provisioned on this build → future updates are silent), and waits for `me` to load before concluding "not verified" so there's no phone-verify flash on updates. OTP send/verify mechanism (Convex `phoneAuthAction`) UNCHANGED (confirmed via integration expert).
- Lint clean; app boots to Sign In. ⚠️ Native/auth-gated — validate on a rebuild: an already-verified account doing an in-place update goes straight to chats; a fresh install/reinstall still asks for phone verification.


## iter-442 (Jun 2026): Call tones "Connected"/"Ciaooo" — audibility fix + black-box diagnostics
- User confirmed iter-440/441 (self-view + scroll) are live on their rebuilt APK, so the patch IS applied — yet the tones are still inaudible. This rules out "stale build"; it's an AUDIBILITY problem during the live WebRTC (Stream) audio session.
- **Fix (`scripts/patch-incallmanager.js`, Android):** switched the tone's `audioUsage` from `USAGE_VOICE_COMMUNICATION` → **`USAGE_VOICE_COMMUNICATION_SIGNALLING`** — the Android-recommended usage for in-call signalling toning, which is designed to MIX with an ongoing voice-communication session rather than compete with WebRTC's exclusive voice stream (why it was silent). Also set full volume (`mp.setVolume(1.0,1.0)` Android / `_smilersTone.volume = 1.0` iOS). Made the patch's "already patched" branches NORMALIZE these lines so the fix applies even if the build reuses a cached, previously-patched node_modules (not just a clean install).
- **Black-box diagnostics (`src/lib/webrtc/inCallManager.ts`):** `playCallConnectedTone`/`playCallEndTone` now `recordDiagnostic({tag:'TONE', …, nativeMethod=<bool>})`. Because `recordDiagnostic` POSTs to `/api/diagnostic-logs`, after the user rebuilds + makes one test call we can read the backend supervisor log (`grep '\[DIAG\]' … | grep TONE`) to confirm (a) the JS tone calls actually fire and (b) whether the native `playInCallSound` method is present — decisively isolating JS-wiring vs native-audibility WITHOUT adb.
- Lint clean; node_modules re-patched locally (verified USAGE_VOICE_COMMUNICATION_SIGNALLING + setVolume present). ⚠️ NATIVE-ONLY — requires a fresh APK/TestFlight rebuild to validate.


## iter-441 (Jun 2026): Chat opens at newest message + double-tap self-view to flip + call-tone volume safeguard
- **Chat "still have to scroll down to reach the latest message" (direct open AND from a notification tap) — FIXED:** the open-at-newest effect in `app/chat/[conversationId].tsx` only retried `scrollToEnd` 6×/130ms (~840ms) then handed control back. A non-inverted virtualized list keeps growing as batches + async media rows render below, so the short window landed partway. Extended to 16×/150ms (~2.5s) AND it now ABORTS the instant the user scrolls themselves (`isUserScrollingRef`) so it never fights a manual scroll. Also added an effect that re-arms `initialScrollDoneRef=false` on `conversationId` change, so a notification tap that reuses the mounted screen with a different conversation re-scrolls to the bottom.
- **Enhancement — double-tap the self-view tile to flip camera (front ⇄ rear):** added double-tap detection inside the existing self-view `PanResponder` (`StreamCallInner.tsx`): a near-stationary release (`|dx|,|dy| < 6`) within 300ms of the previous tap calls `flipCam()` (via `flipCamRef` so the once-created responder always uses the latest callback). Pairs with iter-440's front-only mirror.
- **Call tones ("Connected"/"Ciaooo") — verified wiring is correct + added a volume safeguard:** connect tone fires on the `connected` transition; end tone in `hangup()` gated on `remoteConnected`; both route through the custom native `playInCallSound` (patch-incallmanager.js) exactly like InCallManager's own busytone. Added `mp.setVolume(1.0,1.0)` (Android onPrepared) + `_smilersTone.volume = 1.0` (iOS) to the patch so the one-shot tone plays at full volume on the voice stream. ⚠️ These tones are NATIVE + PATCH-dependent — they ONLY exist in a FRESH APK/TestFlight build (the patch runs at install time). If the user's current build predates iter-428/434 or the volume tweak, a rebuild is required to hear them.
- Lint clean; app boots to Sign In.


## iter-440 (Jun 2026): Stream 1:1 call self-view "steals" remote video + front-camera mirror (P0)
- **ROOT CAUSE (self-view steals the other person's video on camera toggle, 1:1):** the fullscreen remote `ParticipantView` and the overlay self-view `ParticipantView` both rendered with the DEFAULT `videoZOrder=0`. On Android the two native SurfaceViews then compete for the same z-layer, so toggling the local camera off→on re-creates the self-view surface which punches over / steals the remote surface. Fix (`src/components/stream/StreamCallInner.tsx`): remote fullscreen view now `videoZOrder={0}` (background) and the draggable self-view overlay now `videoZOrder={1}` (foreground) — the SDK-recommended split so the self-view composites ON TOP instead of stealing the remote.
- **Front-camera mirror (standard):** read `direction` from `useCameraState()` (`cameraDirection`), derive `selfMirror = direction !== 'back'`, and pass `mirror={selfMirror}` on the self-view — mirrored (selfie) for the front camera, un-mirrored for the rear camera. Matches the legacy WebRTC self-view / pop-out behavior the user asked for. Self-view also given `ParticipantLabel={null}`/`ParticipantVideoFallback={null}` for a clean PiP tile.
- Lint clean; app boots to Sign In. ⚠️ NATIVE-ONLY (Stream WebRTC surfaces) — the self-view layering + mirror can ONLY be validated on an APK/TestFlight build (not web/Expo Go).
...
- **Multi-recipient forward:** `ForwardPickerSheet` is now multi-select — tap chats to pick many (or "Select all"), then "Forward to N chats". `app/chat/[conversationId].tsx`: extracted `forwardMsgsToTarget()` + `collectMsgsToForward()`; new `doForwardToMany(targetIds)` dispatches in **BLOCKS OF 25 targets** (`Promise.allSettled` per block + 350ms gap) with live progress ("Forwarding 25/120…") surfaced in the sheet, then a summary alert ("Sent to N chats · M failed"). Single-select still routes through `doForwardTo`. Preserves E2EE re-encryption per target + the FW<n> lineage counter. Diary stays a one-tap shortcut.
- **Voice typing (hands-free dictation):** new mic button in the composer toolbar (`VoiceTypingButton`, testID `composer-voice-typing`). Uses on-device / platform STT via `expo-speech-recognition` (`src/lib/voiceTyping/useVoiceTyping.ts`): interim + finalized results, finalized text streams into the composer. A live "Listening…" banner shows the partial transcript. On a **5-second pause** a prompt appears — **"Send" / "Keep talking"** (text also stays editable). Long-press the mic to pick the language.
- **Curated languages only:** `src/lib/voiceTyping/voiceTypingLanguages.ts` exposes ONLY high-accuracy dictation languages (English US/UK, Spanish, French, German, Italian, Portuguese BR/PT, Dutch, Russian, Arabic, Hindi, Mandarin, Japanese, Korean, Turkish, Polish); all others are unavailable. Auto-picks the device language if supported (else English); choice persisted in AsyncStorage. Mic/Speech permissions already declared in app.json.
- Lint clean on all files; app boots to Sign In. ⚠️ Voice typing is NATIVE-ONLY (won't work web/Expo Go) — validate on an APK/TestFlight build. Multi-forward works on device (auth + a chat with messages required to validate end-to-end).


- **Root fix for BOTH tones:** the ciao (iter-428) used `InCallManager.stop({busytone})`, but that path is gated behind `audioManagerActivated`/`_audioSessionInitialized` — which is FALSE during a Stream call (Stream owns the WebRTC audio session; InCallManager is never started) → the ciao would have silently no-op'd. Added an isolated, ADDITIVE native method `playInCallSound(bundleName)` to react-native-incall-manager (Android `@ReactMethod` + iOS `RCT_EXPORT_METHOD`) via `scripts/patch-incallmanager.js` (wired into the postinstall/prepare patch chain, idempotent). It plays a one-shot bundled raw sound on the VOICE-COMMUNICATION stream (audible during MODE_IN_COMMUNICATION) WITHOUT starting/tearing down InCallManager's own session, so it never disturbs the live call; the native player keeps running after the RN screen unmounts. Purely additive — no existing method touched, so current (praised) call audio is unchanged.
- **"Connected" tone (deep male voice):** generated "Connected!" via OpenAI TTS (`tts-1-hd`, voice `onyx`, 0.95×) → bundled as `incallmanager_connected.mp3` (assets/sounds + android res/raw + iOS bundle registered in pbxproj). New `InCallAudio.playCallConnectedTone()`. Wired into `StreamCallInner` on the `connected` false→true transition (once per call via `playedConnectedToneRef`): **1:1 → both sides** connect when the callee answers so both hear it; **group → each participant** hears it upon joining (connected = JOINED && a remote present), and the initiator hears it together with the first joiner. Voice & video alike.
- `playCallEndTone()` (ciao) re-routed to the same `playInCallSound('incallmanager_busytone')` so it also actually fires during Stream calls.
- Lint clean; app boots to Sign In. ⚠️ NATIVE-ONLY + requires the new patch → the "Connected"/"Ciaooo" tones can ONLY be validated on a fresh APK/TestFlight build (not web/Expo Go). If a tone is inaudible on device, the fallback is to set MODE_IN_COMMUNICATION explicitly in the native player.


- **Call-end "Ciaooo" (both voice & video):** generated a warm, drawn-out "Ciaooo" via OpenAI TTS (`tts-1-hd`, voice `shimmer`, speed 0.85) and bundled it as `incallmanager_busytone.mp3` in `assets/sounds/`, `android/.../res/raw/`, and `ios/Smilers/` (registered in `project.pbxproj`). New `InCallAudio.playCallEndTone()` in `inCallManager.ts` calls `stop({busytone:'_BUNDLE_'})` — plays the bundle on the native VOICE-CALL stream (audible during MODE_IN_COMMUNICATION teardown, unlike expo-audio; MediaPlayer runs natively so it survives the screen unmount) then auto-stops the session. Wired into `StreamCallInner.hangup` (the single leave point), gated on `remoteConnected` so an unanswered/cancelled ring stays silent. Since the active 1:1 & group path both route through `CallHost → StreamCallInner` (`useStream = !screenOnly && !conference`): 1:1 → both sides run `hangup` (local End OR remote-ended via CallUI `onHangup`) so BOTH hear it; group → only the member who leaves runs `hangup` so only they hear it. Exactly the requested behavior.
- **Auto-Enhance (real, Skia):** installed `@shopify/react-native-skia@2.2.12` (autolink, NO prebuild). New `src/lib/photo/autoEnhance.ts` — lazy-imports Skia (kept out of the web bundle; no-op on web), loads the image onto an offscreen surface, applies a TRUE color-matrix (saturation 1.25 × contrast 1.12 + brightness +0.04), re-encodes to JPEG q92, writes to cache, returns the uri. New **Enhance** tool button (`sparkles`) in `PhotoEditor.tsx` — bakes current annotations (view-shot flatten) then enhances the whole image, clearing annotations/filter like crop/rotate.
- Lint clean on all files; app boots to Sign In (web bundle unaffected by Skia). ⚠️ Both are NATIVE-ONLY — the "Ciaooo" playback (InCallManager busytone) and the Skia enhance can only be validated on an APK/TestFlight build, not web/Expo Go.
...
- **Issue #2b (caller "Calling"/callee "Connecting" spins long / never connects) — ANALYSIS, no blind change:** the call state machine is heavily tuned and praised ("better than WhatsApp once connected"), so I did NOT gamble on its timings. Findings: `connected = CallingState.JOINED && !!remote`; the caller pre-joins the SFU at ring time; room id = `streamRoomParam(push stream_room) || pinnedRoom(callId/createdCallId)`. "Never connects" is most likely (a) a BACKEND room-id contract mismatch — the FCM `stream_room` the callee joins must equal the caller's call-record id — or (b) flaky-network join failures (join has a 14s watchdog + 1 retry; if both fail it dead-ends on "Connecting…"). Needs a two-device diagnostic (the STALL log already prints both sides' room ids) or backend confirmation of the `stream_room` value before a precise fix. Communicated to user.
...
- **Issue #1 "auto backup never runs" — the `/backup` screen was a PLACEBO; now REAL (user chose 1a):** the old `onBackupNow` just waited 1.2s and wrote a fake `lastBackupAt`; nothing was backed up and there was NO scheduler.
  - New `src/lib/backup/chatBackup.ts`: `buildChatBackupPayload(convex, opts, onProgress)` walks EVERY conversation, paginates ALL messages via `continueCursor`, pulls call logs, honours include-toggles (media/voice/docs; text always kept); `writeEncryptedChatBackup` AES-GCM-256 + PBKDF2 envelope (same crypto + App Lock PIN as Diary) to `documentDirectory/SmilersBackups/*.smchat.json` (retains newest 4); `decryptChatBackup`, `chatBackupToReadableText`, `listLocalChatBackups`/`getLatestChatBackupUri`, and `maybeRunAutoChatBackup` (opportunistic scheduler: due-check by frequency, Wi-Fi-only via NetInfo, PIN-gated, never throws).
  - `app/backup.tsx`: real "Back up now" (progress label + share the encrypted file), new "Restore from backup" (decrypt latest/local or picked file → readable archive share; honest copy that restore recovers a READABLE copy since chats can't be re-injected into the external Convex backend), truthful "Last backup" (max of settings + newest local file), fixed misleading tip copy.
  - Auto-backup trigger wired into `app/(tabs)/chats.tsx` (9s-deferred on app open, guarded by settings+PIP+due-time). NOTE: exact "2 AM local time" isn't reliably possible on mobile (OS background limits) — this runs on next app-open when due; user was informed.
  - Lint clean on all files; app boots to Sign In. ⚠️ Backup/restore/pagination are native + session-gated — validate on the APK rebuild with a signed-in account + App Lock PIN set.
...
- **P0 iOS build blocker FIXED (`no such module 'stream_io_video_react_native'` at AppDelegate.swift:5):** the previous agent's manual VoIP wiring used a WRONG Swift module name. The pod is `stream-video-react-native` (Swift module `stream_video_react_native`), and `StreamVideoReactNative` is an **Objective-C** class exposed via the bridging header — NOT importable as `stream_io_video_react_native`. Fix (matches the official `@stream-io/video-react-native-sdk` config plugin, keeping the current non-`use_frameworks` static linking so react-native-webrtc/Twilio/Stream-WebRTC aren't disturbed):
  ...
  2. `ios/Smilers/Smilers-Bridging-Header.h` (was EMPTY): added `#import "StreamVideoReactNative.h"` so `StreamVideoReactNative.voipRegistration()` compiles. NEVER ran prebuild (custom native folders preserved).
- **Self-view flicker + "caller's video not working" — root cause from the user's `smilers-self-view-log.txt`:** 1:1 calls run through **Stream** (`CallHost` → `StreamCallInner`), constraints 1280x720@30 confirm Stream SDK. Log shows the local camera FULLY torn down (Camera2 disconnect) + re-`getUserMedia` ~3× in a 2-min call (at start 29.7s→31.5s double, then ~every 60s on Stream reconnects) → self-view goes black/flickers on each re-acquire, and the outgoing publish can drop so the peer receives only audio (`onAddTrack` = audio-only; remote never published video). Multiple effects each fired `call.camera.enable()` back-to-back and again on every reconnect, compounding the churn.
- **Fix (`src/components/stream/StreamCallInner.tsx`):** added `useCameraState()` + a guarded `ensureCameraOn()` that (a) NO-OPs when the camera is already `status==='enabled'` and (b) coalesces rapid repeat enables (1200ms min gap). Routed all three enable call-sites (connect re-assert, AppState-resume, isVideo-init) through it; kept the delayed re-assert (now 800ms, idempotent) and the voice-path `camera.disable()`. Eliminates the self-inflicted redundant-enable churn; the same fix on the peer's device stabilises their publish so remote video arrives.
- ⚠️ Both fixes are **native-only** — validate on a fresh EAS iOS build (module error gone) and an APK/IPA rebuild (self-view steady, remote video shows). The ~60s Stream re-publish on weak networks is SDK/network-driven and only partially in-app-controllable.
...
## iter-fork — Android sign-in loop, Voice Typing caps/grammar, Study Room reader, Bluetooth auto-route
- **Android "shaking / repeatedly opening sign-in" loop FIXED**: `src/providers/ConvexClientProvider.tsx` auth-watchdog auto-called `trySilentReauth()` (OIDC prompt=none) which presents the native auth browser → flash open/close → background/foreground → re-trigger → violent loop. Removed the AUTOMATIC silent-reauth (stage 2a). Invisible heals remain (stage-1 token refresh via refreshAsync + client recreate). `trySilentReauth` now runs ONLY on the manual "Reconnect" button (chats empty state). Removed unused SILENT_REAUTH_COOLDOWN_MS/lastSilentAt.
- **Voice Typing caps + grammar DONE**: new `src/lib/voiceTyping/capitalize.ts` (liveCapitalize + applyCaps, on-device regex). `app/study/voice-typing.tsx`: CAPS ON/OFF toggle (non-lossy — buffer stays natural-cased), live capitalization on commit, grammar-adaptive polish via shared Convex action `study.voiceTypingGrammar.polish` (runs before copy prompt + manual "Fix grammar & capitals" button; allCaps arg mirrors CAPS toggle; best-effort try/catch).
- **Study Room "Read with all" — partial**: fixed client leader-election bug in `app/study/scripture.tsx` (was `isLeader = mode==='all'` → EVERY member became a leader, nobody followed). Now room admin/chief = sole broadcaster (via useRoom role), others follow; badge shows "Following". BLOCKER: study rooms have NO live Stream call, and current transport (`src/lib/scripture/sync.ts`) broadcasts over a Stream call → nothing to transmit. Needs Convex backend fns (user's web team, 2a agreed): `study.rooms.setReadingPosition({roomId,material,translation?,edition?,book?,chapter?,surah?,scrollPct,ts})` (admin only) + `study.rooms.getReadingPosition({roomId})` reactive. Then wire mobile leader-writes-on-scroll / followers-subscribe.
- **Bluetooth auto-detect & auto-route (all call stacks)**: WebRTC 1:1 (`app/call/[conversationId].tsx`) + Twilio (`app/twilio-call.tsx`) already had it. Added to Stream 1:1/group (`src/components/stream/StreamCallInner.tsx` — routeAudioNative/userPickedRouteRef/btWasAvailableRef + connected-effect: ensureBluetoothPermission + addAudioDeviceChangedListener auto-route, fallback speaker(video)/earpiece(voice); doEnableVideo keeps BT). Added to mesh group-call (`app/group-call/[conversationId].tsx` — device listener → setBluetoothOn / fallback speaker). All Android-only; iOS AVAudioSession auto-routes. NATIVE-only, needs device build to validate.
- iOS TestFlight splash fix STILL PENDING: Podfile now force-loads EACH Expo module static lib (not just aggregate) — awaiting EAS build + module-inventory beacon (expect present=20/20).

## iter-463 (fork) — iOS permission "Unrecognized requester" ROOT-CAUSE reframe + Podfile safety-net
- **Previous "user is building stale code" conclusion was WRONG — user exonerated.** Proof: this fork's backend URL is `smilers-launch.preview.emergentagent.com`; its supervisor logs never received telemetry from the user's real builds (2.2.50–2.2.56) — highest version ever seen is 2.2.18, recent logs only from `web` preview. Support confirmed preview vs deployment envs are ISOLATED, so a TestFlight build bakes in a DIFFERENT backend URL and its `PROBE`/`HB` logs go THERE, not to the fork. That is the ONLY reason the agent couldn't see PROBE logs.
- **User on-device `Settings → Diagnostic Logs` screenshot (build 2.2.56, iOS 26.4.1) confirms:**
  - Baked-in `API: app-migration-75.emergent.host` (a real deployed backend — NOT the fork URL). App boots fine (59 events, splash OK → FIX 1 works).
  - REAL bug reproduces: `NOTIFY emergentPush/register → register failed: Unrecognized requester: EXUserFacingNotificationsPermissionsRequester`.
  - Separate backend bug seen: `CONVEX M(users:updateProfile) Server Error` on `backupSettings` (Diary backup save — web team / Convex owned).
- **Native mechanics verified in node_modules/expo-modules-core/ios/Core:** `AppContext.permissions` = `legacyModuleRegistry?.getModuleImplementingProtocol(EXPermissionsInterface)`. `useModulesProvider` → `ModuleRegistry.register(fromProvider:)` → `moduleType.init(appContext:)` → NEW ModuleHolder → re-runs `OnCreate {}` (where requesters register). So re-registration DOES re-attempt requester registration IF `permissions` (legacyModuleRegistry) is non-nil at that time. Upstream already re-registers in `legacyProxyDidSetBridge:`. Requesters failing means: patch/Podfile-revert NOT compiled into the build (postinstall skipped or Pods cached) OR legacy registry never populates in this bridgeless setup.
- **FIX APPLIED (safest, no fragile native rewrite): `ios/Podfile` pre-`prepare_react_native_project!` safety-net** runs `node ../scripts/patch-expo-modules-race.js` during `pod install`. `pod install` ALWAYS runs on the build server (no committed Pods/ or Podfile.lock), so the expo-modules-core race/permission patch is now GUARANTEED to compile into every build regardless of whether Emergent runs npm postinstall/prepare. Verified idempotent (3 early sites, 1 retry method, no dup, 130 lines). Mirrors why SmilersCallModule was committed to native source.
- **NEXT (pending user): Publish → fresh iOS build → install → open app → Settings → Diagnostic Logs → check if `Unrecognized requester` is gone.** If gone → Issue 1 closed. If it persists after a build carrying the Podfile safety-net, the bridgeless registration APPROACH itself is wrong and needs replacing (deeper native strategy / Expo issue) — the "postinstall skipped" and "stale build" hypotheses will both be eliminated.
- **DIAGNOSTIC NOTE for future forks:** never conclude "stale build" from missing backend telemetry in a fork — the build's baked `EXPO_PUBLIC_BACKEND_URL` differs from the fork's. Use the ON-DEVICE `Settings → Diagnostic Logs` screen (reads AsyncStorage, no backend) as the source of truth.

## iter-463 (fork) — CONTINUED: useFrameworks:static is the real conflict + BUILD-MARKER gate
- User confirmed the build that "still had no change" was generated AFTER the Podfile safety-net → Podfile patch-guarantee alone did NOT fix `Unrecognized requester`.
- **Root conflict identified:** `ios.useFrameworks: "static"` is set in app.json (expo-build-properties plugin, ~line 257) AND ios/Podfile.properties.json. The iter-460 Podfile comment says framework/static linkage breaks Expo permission requesters — but the previous agent only removed the per-pod `-force_load` override, NOT the underlying `use_frameworks! :static`.
- **BUT useFrameworks:static CANNOT be removed:** `@stream-io/noise-cancellation-react-native` depends on `StreamVideoNoiseCancellation` (binary Swift xcframework pod) which requires use_frameworks. Removing it would break the Stream call noise-cancellation stack. So the fix must make Expo permission requesters register DESPITE static frameworks.
- **Likely mechanism (unverified, needs device build):** with static frameworks + dead-code stripping, the legacy `EXModuleRegistryProvider` ObjC-runtime scan may not discover/keep the `EXPermissions` service class → `appContext.permissions` stays nil → requesters skipped → "Unrecognized requester". Note: Podfile sets `DEAD_CODE_STRIPPING=NO` only on the APP target, NOT on the Pods/Expo pod targets. Candidate real fix: reinstate a TARGETED `-force_load`/`-ObjC` for the Expo permissions/interfaces pod, or disable dead-code stripping on expo pods too — but this is the exact area iter-460 said it "broke views/requesters," so it's contradictory and MUST be verified on a real build before trusting.
- **GATE BEFORE ANY MORE NATIVE CHANGES — BUILD-MARKER test:** added `BUILD-MARKER=iter463-KX7Q9` to the BOOT diagnostic in app/_layout.tsx (line ~141). User to build + check Settings→Diagnostic Logs.
...
## iter-463c (fork) — CODE PROPAGATION CONFIRMED + real fix: touch NativeUnimoduleProxy
- **SETTLED: fork code DOES reach the user's iOS builds.** On-device PROBE showed `[iter463b-OBJC]` marker → every fix lands. Stop doubting propagation.
- `-ObjC` (iter463b) built cleanly but did NOT fix requesters → not a stripping problem.
- **True root cause (traced in expo-modules-core source):** `EXNativeModulesProxy` (RCT_EXPORT_MODULE `NativeUnimoduleProxy`) is the ONLY thing that calls `legacyProxyDidSetBridge:` → sets `appContext.legacyModuleRegistry` → makes `appContext.permissions` (EXPermissionsService) non-nil so module OnCreate can register requesters. In RN 0.81 bridgeless, legacy bridge modules are created LAZILY (only when JS touches them). Nothing touches NativeUnimoduleProxy → registry stays nil → EVERY requester fails "Unrecognized requester" (imagepicker/camera/contacts/location/notifications, all delays to 10s).
...
## iter-463e (fork) — ROOT CAUSE CONFIRMED + REAL FIX: force-link EXPermissionsService
- Native diag (iter463d) captured EXACTLY ONE native event: "legacyProxyDidSetBridge fired". NO "setRuntimeExecutor early register" and NO retry events → in this bridgeless build setRuntimeExecutor path doesn't run; registration happens via legacyProxyDidSetBridge → useModulesProvider, WITH legacyModuleRegistry already set.
- Therefore registry IS wired, yet requesters fail → `appContext.permissions` (EXPermissionsInterface via legacy registry) is nil → EXPermissionsService MISSING from registry.
- CONFIRMED mechanism: EXPermissionsService registers via ObjC `+load` (EX_EXPORT_MODULE → EXRegisterModule into EXModuleClasses). Under `use_frameworks! :static` the linker drops its object file (unreferenced) so `+load` never fires. `-ObjC` (iter463b) does NOT reliably force-load classes from static FRAMEWORKS (only static libraries) → didn't help.
- FIX SHIPPED (patch-expo-modules-race.js FIX 5): import `<ExpoModulesCore/EXPermissionsService.h>` + `(void)[EXPermissionsService class];` inside ExpoBridgeModule.mm legacyProxyDidSetBridge (right after setting legacyModuleRegistry, before useModulesProvider). ExpoBridgeModule.o IS linked → the class reference creates an undefined symbol → linker pulls EXPermissionsService.o (same static framework) → its +load fires at launch → service registered in EXModuleClasses → in the registry → appContext.permissions non-nil → module OnCreate registers requesters. Self-contained in ExpoModulesCore; guaranteed compiled via the Podfile pod-install safety-net. Also appends native diag "forced EXPermissionsService link; class=…". Probe BUILD tag → iter463e-PERMFIX.
- Kept -ObjC (harmless). NEXT build check Settings→Diagnostic Logs: PROBE `[iter463e-PERMFIX] … OK status=…` → FIXED (permissions work). NATDIAG should show "forced EXPermissionsService link; class=EXPermissionsService". If still ERR → other legacy services also stripped; extend force-link list (but EXPermissionsService is the sole permissions gate, high confidence).

## iter-463f (fork) — FIX 5 worked (service present) → FIX 6 timing re-register
- iter463e result: NATDIAG showed "forced EXPermissionsService link; class=EXPermissionsService" AND "Unrecognized requester" is thrown by EXPermissionsService.m:61 (service EXISTS + is queried) → FIX 5 SUCCEEDED (service now linked/registered). Remaining gap: requesters never registered INTO the service (_requestersByClass empty). Confirmed ModuleHolder.init posts .moduleCreate (OnCreate runs at registration) — so OnCreate ran while appContext.permissions was still nil (silent `?.` skip), service resolved only afterwards (empty).
- FIX 6 (patch FIX 6): after useModulesProvider in legacyProxyDidSetBridge, diagnose `getModuleImplementingProtocol(EXPermissionsInterface)` + internalModules count; if nil call `[moduleRegistry initialize]`; once resolved RE-run `useModulesProvider` so OnCreate re-runs WITH permissions available → requesters register. Imports EXPermissionsInterface.h + EXModuleRegistry.h. Diags: "afterUseModules: permSvc=… internalModules=…", "afterInitialize: permSvc=…", "re-registered modules after permSvc resolved".
- AppDelegate.swift: clears NSUserDefaults key smilers_native_diag at launch top so log shows only current launch. Probe BUILD → iter463f-REREG.
- NEXT build check: PROBE `[iter463f-REREG] … OK` → FIXED. Else read NATDIAG: if "afterUseModules: permSvc=RESOLVED" but still ERR → two-instance/OnCreate issue (requesters register into a different service instance than probe queries); if permSvc=nil→RESOLVED after initialize + "re-registered" but still ERR → OnCreate not re-running requester registration. Method names verified against EXModuleRegistry.h (getModuleImplementingProtocol:, getAllInternalModules, initialize).

## iter-463g (fork) — ROOT BLOCKER: ExpoModulesCore pod NOT recompiling (native compile cache)
- iter463f result: NATDIAG showed ONLY "forced EXPermissionsService link" (FIX5) + "legacyProxyDidSetBridge fired" — FIX6's "afterUseModules"/"re-registered" diagnostics were ABSENT. Meanwhile the COMMITTED AppDelegate.swift change (clear NSUserDefaults at launch) TOOK EFFECT (stale entries gone). => committed app-target recompiles every build, but the expo-modules-core POD does NOT recompile when only its patched source changes.
- Podfile line 1 uses `node --print`, proving node IS available during pod install → my Podfile patch hook DOES apply FIX6 to build-server node_modules. So FIX6 SOURCE is present; the compiled ExpoModulesCore framework is CACHED (ccache/DerivedData/Pods artifact reuse) and not rebuilt. Support: undocumented infra; escalate to support@emergent.sh with job id.
- SELF-SERVICE FIX (committed, reliable): ios/Podfile post_install now injects a per-revision compiler flag on the ExpoModulesCore target only (`GCC_PREPROCESSOR_DEFINITIONS += SMILERS_PERM_PATCH_REV=463g` and `OTHER_SWIFT_FLAGS += -DSMILERS_PERM_PATCH_REV_463g`). Changing the pbxproj build settings each rev should force Xcode to rebuild that target (and bust ccache via changed compile command), so the freshly-patched ExpoBridgeModule.mm (FIX5+FIX6) actually recompiles. Bump REV whenever the pod patch changes.
- Probe BUILD → iter463g-CACHEBUST. NEXT build check: if NATDIAG now shows "afterUseModules: permSvc=… internalModules=…" (and hopefully "re-registered modules after permSvc resolved") → the pod recompiled → FIX6 active; then PROBE should turn OK (or the afterUseModules diag tells us permSvc RESOLVED-but-empty = instance mismatch). If afterUseModules STILL absent → pipeline artifact cache defeats build-setting invalidation → ESCALATE to support@emergent.sh (undocumented native build cache) — no further self-service option.
- FULL ROOT CAUSE (confirmed, for escalation): EXPermissionsService resolves via legacy registry only AFTER module OnCreate runs (OnCreate registers requesters via appContext.permissions?.register but permissions was nil then → skipped → service ends up empty → "Unrecognized requester" from EXPermissionsService.m:61). FIX = re-run useModulesProvider after the service resolves (FIX6). Blocker = getting FIX6 to actually recompile.

## iter-463g RESULT — cache-buster FAILED → HARD BLOCK (escalate to Emergent)
- afterUseModules (FIX6) STILL absent after Podfile per-rev compiler-flag cache-buster. Only FIX5 (compiled iter463e) + legacy-log appear. => pipeline reuses a PREBUILT/cached ExpoModulesCore product that ignores BOTH source changes AND ExpoModulesCore-target build-setting changes. Confirmed not RNcore-prebuilt (that's RCT_USE_PREBUILT_RNCORE, RN core only); expo-modules-core builds from source pod, so this is a pipeline artifact/DerivedData/Pods cache. Undocumented — support says escalate to support@emergent.sh with job id app-migration-75.
- Committed app-target fix NOT viable: shared EXAppContext (needed to call useModulesProvider) has no public/global accessor reachable from AppDelegate/ExpoReactNativeFactory; a local Expo module would need autolinking config (prebuild-ish) — disallowed.
- STATE OF REPO (ready for when cache is cleared): FIX1-6 in scripts/patch-expo-modules-race.js (all correct), Podfile safety-net + -ObjC + ExpoModulesCore cache-buster, committed SmilersCallModule.getNativeDiag bridge, AppDelegate NSUserDefaults clear, permissionRequesterProbe (BUILD iter463g-CACHEBUST) + NATDIAG reader. Once Emergent forces a CLEAN pod rebuild, FIX6 should register requesters (root cause fully confirmed: EXPermissionsService resolves only AFTER module OnCreate → requesters skipped → re-register once resolved).
- NEXT AGENT: do NOT burn more build cycles on native pod patches until Emergent confirms a clean rebuild happened. Verify via NATDIAG "afterUseModules: ..." appearing. If a NEW pod-patch rev is needed later, bump SMILERS_PERM_PATCH_REV in Podfile AND the probe BUILD tag.

## iter-464 (fork) — Google Play target API 36 (Android 16) compliance
- Google Play Console: "App must target Android 16 (API 36) or higher" by Aug 31, 2026 (app currently targets API 35). Bumped targetSdk/compileSdk 35→36.
- FIX (native-folder-safe, no prebuild): android/gradle.properties += android.compileSdkVersion=36, android.targetSdkVersion=36, android.buildToolsVersion=36.0.0. Expo SDK 54 ExpoRootProjectPlugin reads these gradle props (same mechanism as existing android.minSdkVersion=24) via ExpoAutolinkingSettingsExtension mapping; defaults to 35 if unset. app/build.gradle consumes rootProject.ext.{compileSdkVersion,targetSdkVersion,buildToolsVersion}.
- Also mirrored into app.json expo-build-properties android {compileSdkVersion:36,targetSdkVersion:36,buildToolsVersion:"36.0.0"} for config parity (plugin itself is inert without prebuild).
- Did NOT touch versionCode/versionName (build pipeline manages them). Cannot verify Android build in-env — needs a Publish→Android build. Verify: built AAB targetSdk=36, then upload to Play (internal/closed testing) → Play accepts.

## iter-465 (fork) — Call-end: notification tone covering "Ciao" + Ciao volume
User: on every call end a notification with the Smilers tone covers the "Ciao" (call-ended) tone; also make Ciao as loud as "Connected".
- VOLUME: inCallManager tones play at volume 1.0 on both iOS (SmilersCallModule.swift) & Android (SmilersCallModule.kt). Measured source loudness: busytone(Ciao) mean -18.9dB vs connected -19.1dB — already equal. Still boosted Ciao via ffmpeg (volume=4dB + alimiter limit=0.97) → mean -15.4dB, max -0.5dB (clearly ≥ Connected). Replaced ALL 3 copies: ios/Smilers/, android/app/src/main/res/raw/, assets/sounds/incallmanager_busytone.mp3 (same md5 0e3d9ab...). pbxproj already references ios copy.
- NOTIFICATION (strong candidate): src/lib/notification/useMessageNotificationSound.ts plays message_notification_beep + smilers_notification tone when a conversation lastMessageTime increases & not-in-that-chat & !lastMessageFromMe. Every call end inserts a call-log message → this fires right as the Ciao plays → covers it. FIX: inCallManager.ts now exports markCallEnded()/getLastCallEndedAt() (markCallEnded called inside playCallEndTone); useMessageNotificationSound skips playSound() if Date.now()-getLastCallEndedAt()<6000ms (baseline map still updated so no late play). This is in-app FOREGROUND sound suppression (foreground push handler already sets shouldPlaySound:false for all types).
- VERIFY on next iOS+Android build: end a call → hear Ciao clearly with NO Smilers notification tone over it. If a visible push BANNER also appears (app backgrounded during call teardown), that's a separate push-side issue — capture Diagnostic Log to identify (NOTIFY/notifee events).


## iter-466 (fork) — False missed-call on answered Stream calls + Stream call background + call-waiting overlay polish
User: (1) answering a call (Stream/native-notification) then hanging up still fires a false "Missed call" push; (2) wire the blue gradient call background into the Stream call screen; (3) Stream call-waiting parity with WebRTC.

**Issue 1 — False "Missed call" (P0) — FIXED (frontend, device-validate):**
- ROOT CAUSE: `useIncomingCallListener.ts` gates its missed-call debounce on `userAnsweredRef`, which is ONLY set on the Twilio/WebRTC foreground answer path (lines 298/349). A call answered via the native notification "Answer" button OR the Stream in-app UI never sets it → ringing record disappears on hang-up → debounce fires `cancelRingAndShowMissedCall()` → false missed-call push.
- FIX: added an "answered recently" registry to `src/lib/call/activeCallRegistry.ts` (`markCallAnswered`, `wasCallAnsweredRecently`, 2-min TTL, keyed by both callId & conversationId). `setActiveCall()` now auto-records into it whenever a call goes active. `StreamCallInner` now calls `setActiveCall({callId, conversationId})` on `accepted` and clears on unmount (previously ONLY the legacy WebRTC screen `app/call/[conversationId].tsx` did). The missed-call debounce in `useIncomingCallListener.ts` now checks `wasCallAnsweredRecently(snapshot._id, snapshot.conversationId)` before firing — so a hung-up-after-answer is never mistaken for a missed ring, regardless of answer path.

**Task 2 — Stream call background — DONE:** wired `<CallBackground variant="incoming" />` (blue gradient, src/components/CallBackground.tsx) behind BOTH the Stream "Connecting…/Calling…" loading screen AND the in-app incoming Accept/Decline screen in `StreamCallInner.tsx`.

**Task 1 — Stream call-waiting — SHIPPED as option (b) clean overlay (NOT true hold):**
- RESEARCH FINDING (Stream RN SDK docs): true concurrent-call HOLD is architecturally unsupported by Stream. Our client sets `rejectCallWhenBusy:true`; there is NO per-call "mute incoming remote audio" API (two joined calls would both play audio); the only concurrency pattern is leave→rejoin, which on 1:1 drops the held party. WebRTC's `useSecondaryCall` hold works only because we own the raw peer connection (mute remote track locally) — Stream gives no such hook. Documented so future forks don't re-attempt.
...
## iter-469 (fork) — Multi-party (3+) audio/video + tags not rendering for added-participant calls
User (on-device, "scammers blacklist" 3-person call): (a) 3rd+ person connected but can't be heard by the first two; (b) in video only the first two's videos are visible; (c) Joined/Left/Missed/Dial-again tags not rendering. Screenshot = roster modal "In this call (3)".

ROOT CAUSES (both frontend, `StreamCallInner.tsx` `CallUI`):
1. **(a)+(b):** CallUI was a 1:1 layout — `const remote = remoteParticipants[0]` — it only rendered the FIRST remote. Everyone rendered a single `ParticipantView`, so the 3rd+ participant's video was never shown AND their tracks were never registered for Stream subscription (dynascale subscribes tracks for RENDERED ParticipantViews), so others didn't hear/see them. "In this call (3)" uses live participantCount (remoteParticipants+1)=3 → they ARE all in one SFU room, confirming this is a render/subscription bug, NOT the streamRoom bug.
   FIX: added a multi-party GRID branch in `remoteArea` — when `remoteParticipants.length > 1` (and not screen-sharing), render EVERY remote in a flex-wrap grid of ParticipantViews (video tile, or avatar fallback for voice/cam-off), each with a name badge. Tile size adapts (≤2 → 50%×100%, ≤4 → 50%×50%, else 33%). This registers all remotes for track subscription → fixes both audio and video. Single-remote 1:1 layout unchanged. New styles: gridWrap/gridTile/gridFallback/gridAvatar(+Text)/gridNameBadge(+Text).
2. **(c):** ALL status/tag/redial/joined-report/left-detect logic was gated on `isGroupCall`, but an added-participant call on a DIRECT/1:1 conversation is NOT a group conversation → `isGroupCall === false` → nothing rendered ("scammers blacklist" showed as "3 people", non-group).
   FIX: new `isConferenceCall = isGroupCall || remoteParticipants.length > 1 || roster has any other member`. Replaced `isGroupCall` with `isConferenceCall` for: participants-pill counts, waiting strip, roster modal status tags + Dial-again + "Call again", the "report my joined" effect, and the Left auto-detect effect. Group-ADMIN-specific logic (kick, add-request approval, remove authorization) stays on `isGroupCall`. Outer hangup/decline self-reports (`left`/`declined`) now fire on `isGroupCall || !!streamRoomParam` so an added participant who was invited (streamRoomParam set) reports their own leave/decline too.
...
## iter-472 (fork) — one-way audio self-heal, paginated call grid, photo zoom, chat lands-on-latest
User (on device): (1) added participants can hear the first two, but the first two can't hear the added ones; (2) video thumbnails too large — fit as many per page as possible + swipe for extra pages; (3) opening a photo should support pinch zoom; (4) opening a conversation should LAND on the latest message, not visibly scroll down.

1. **One-way audio self-heal (`StreamCallInner.tsx`):** symptom = others can't hear the person who answered (often from the notification) → that person's local MIC track isn't publishing while they still SUBSCRIBE (so they hear others). Added a self-healing effect: when connected + micOn but `isParticipantMuted(local)` is still true ~2.5s after connect, force `microphone.disable()→enable()` to re-publish. Gated on actual publish state (never churns a healthy call) and capped at 2 attempts. NOTE: this fixes the PUBLISH-side cause. DIAGNOSTIC for the user: on the first two phones, does the added person show the red mic-off/"Muted" badge (from iter-470)? If YES → publish issue (this fix targets it); if NO (shows unmuted but silent) → it's an SFU/subscribe issue needing a different fix. Device-only.
2. **Paginated participant grid (`StreamCallInner.tsx`):** replaced the flex-wrap grid with `ParticipantGridPager` — a horizontal `pagingEnabled` FlatList. Up to `GRID_PER_PAGE=9` tiles/page, auto columns (1/2/3) + rows sized to that page's count, swipe left/right for more, page dots (`pagerDots`). Tiles keep the mute icon + active-speaker highlight. `styles.gridWrap` now unused (left in place).
3. **Photo pinch-zoom:** new `src/components/ZoomableImage.tsx` (gesture-handler + reanimated): pinch-zoom, pan while zoomed, double-tap 1×⇄2.5×, single-tap (only at 1×) closes. Wrapped in its own `GestureHandlerRootView` so gestures work inside the RN Modal on Android. Wired into `MediaBubble.tsx` full-screen image viewer (replaced the static `<Image>`/backdrop Pressable). Save/Share/Use-as-profile toolbar unchanged.
4. **Chat lands-on-latest (`app/chat/[conversationId].tsx`):** the non-inverted list snapped to bottom via visible batch scrolls. Added a `listReady` opacity gate: list renders at `opacity:0` until it has snapped to the newest message, then reveals. Revealed via a debounced (220ms) scrollToEnd in `onContentSizeChange`, plus safety reveal when the initial-scroll retries finish or the user scrolls; reset to hidden on `conversationId` change. Deep-link/jump opens reveal immediately.
